...
function event_socket_create($host, $port, $password) { // MOD by BAYU
        $socket_ip = $_SESSION['domain_name'];
        $sql = "select g.freeswitch_endpoint,g.freeswitch_endpoint_port,g.freeswitch_psw,g.public_freeswitch_endpoint ";
        $sql .= "from v_domains as g where g.domain_name='$socket_ip'";
        $database = new database;
        $socks= $database->select($sql, null, 'all');
        unset($sql);
        $host=$socks[0][freeswitch_endpoint];
        $password=$socks[0][freeswitch_psw];
	$port=$socks[0][freeswitch_endpoint_port];
// END MOD
