								...
								if ($tier_row['agent'] == $agent_row['name']) {
									...
									if (permission_exists('call_center_active_options')) {
										...
										if ($tier_state == "Offering" || $tier_state == "Active Inbound") {
											...
											echo "$agent_uuid - $agent_extension " .button::create(['type'=>'button','class'=>'link','label'=>$text['label-eavesdrop'],'onclick'=>"if (confirm('".$text['message-confirm']."')) { send_command('call_center_exec.php?command=eavesdrop&uuid=".urlencode($agent_uuid)."&extension=".urlencode($agent_extension)."'); } else { this.blur(); return false; }"]);
